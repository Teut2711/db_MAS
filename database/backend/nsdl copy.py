...
class ProcessDf:
    #  The following class takes in a pandas dataframe and creates/updates the tables
...
def main(file_obj):
    ...
    # Count columns initially in database
    ...
    dematad = bool(models.Dematad.objects.exists())
    demathol = bool(models.Demathol.objects.exists())
    
    
    #t1 = time()      
    
    for df in GetDataFrame.read_file_obj(file_obj, cols_df):
        # if table does not exists then duplicates are handling by keep_first of pandas
        if not(dematad):
